...
def withdraw_method_keyboard():
    ...
    for method in enabled_methods:
        if method == "BKASH":
            ...
            buttons.append([KeyboardButton("🪙 BINANCE (UID)")])  # ← CHANGED
...
def is_valid_binance_uid(number):
    """Validate Binance UID (8-10 digits)"""
    number = re.sub(r'\D', '', str(number))
    return len(number) >= 8 and len(number) <= 10 and number.isdigit()
# ========== END NEW FUNCTION ==========
    number = re.sub(r'\D', '', str(number))
    return len(number) == 11 and number.startswith('01')
                    ...
                    if num in active_numbers and sms_key not in paid_keys_set:
                        ...
                        group_buttons = InlineKeyboardMarkup([
                            [
                                ...
                                InlineKeyboardButton("📢 CHANNEL", url="https://t.me/+H_ptePj4WXNiMWQ1", style="success")
                            ]
                        ])
                        try:
...
async def fast_allocate_number(query, context, rid, service, range_display):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📢 OTP GROUP", url="https://t.me/+N7r0U3fEb6o0NTVl", style="primary")],
...
async def withdraw_method_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in method_map:
        ...
        # Show method-specific instruction
        if method == "BINANCE":
            instruction = "📌 পরে আপনাকে আপনার **Binance UID** (8-10 ডিজিট) দিতে হবে।"
        else:
            instruction = "📌 পরে আপনাকে আপনার **ফোন নম্বর** (01XXXXXXXXX) দিতে হবে।"
...
async def withdraw_number_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if method == "BINANCE":
        # Validate Binance UID (8-10 digits)
        if not (len(clean_number) >= 8 and len(clean_number) <= 10):
            await update.message.reply_text(
                "⚠️ **Invalid Binance UID!**\n\n"
                "📌 Binance UID should be 8-10 digits.\n"
                "🔢 Example: `123456789`\n\n"
                "Please send your correct Binance UID:",
                parse_mode="Markdown",
                reply_markup=cancel_keyboard()
            )
            return
        payment_type = "Binance UID"
    else:
        # Validate Bangladesh number (11 digits starting with 01)
        if not (len(clean_number) == 11 and clean_number.startswith('01')):
            await update.message.reply_text(
                "⚠️ **Invalid Number!**\n\n"
    ...
    msg = (
        f"✨ <b>YOUR PAYMENT DETAILS!</b> ✨\n\n"
        f"<blockquote>🆔 USER ID: <code>{uid}</code>\n"
        f"📝 METHOD: {method}\n"
        f"📞 {payment_type}: {payment_number}\n"
...
async def process_withdraw_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    payment_type = temp_data.get("payment_type", "Phone Number")  # ← NEW
    ...
    await query.message.edit_text(
        ...
        f"📞 {payment_type}: <code>{payment_number}</code>\n"  # ← CHANGED
...
async def admin_approve_withdraw(update, context, payment_id):
    ...
    payment_type = rd.get("payment_type", "Phone Number")  # ← ADD THIS LINE
    ...
    await query.message.edit_text(f"✅ APPROVED | User: {uid} | {payment_type}: {payment_number} | Amount: {format_balance(amount)} BDT")
...
async def admin_reject_withdraw(update, context, payment_id):
    ...
    amount = rd["amount"]
    payment_type = rd.get("payment_type", "Phone Number")  # ← ADD THIS LINE
    payment_number = rd.get("number", "N/A")  # ← ADD THIS LINE
